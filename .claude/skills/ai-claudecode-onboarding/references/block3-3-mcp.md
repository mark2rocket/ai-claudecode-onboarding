...
> 💡 **용어 정리**
> - **MCP(Model Context Protocol)**: Claude와 외부 앱이 서로 대화할 수 있게 연결해주는 표준 규격. 스마트폰에 앱을 깔듯이, Claude에 기능을 추가하는 방식
> - **프로토콜**: 서로 다른 서비스가 대화하기 위한 약속된 방식 (규격)
...
> 💡 **용어 정리**
> - **툴 콜링(도구 호출)**: Claude가 텍스트를 생성하는 데 그치지 않고, 실제로 외부 서비스에 "이 작업 해줘"라는 명령을 직접 보내는 것
...
### 1인 라이프스타일 크리에이터에게 유용한 MCP 예시

| MCP | 가능한 일 |
|-----|----------|
| Notion MCP | 콘텐츠 캘린더 자동 업데이트, 아이디어 기록 |
| Google Calendar MCP | 촬영·편집·업로드 일정 확인 및 추가 |
| YouTube MCP | 조회수·댓글·구독자 데이터 가져오기 |
| Slack/Discord MCP | 팀·커뮤니티 메시지 자동 수집 |
