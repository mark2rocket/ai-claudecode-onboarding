# Session 4: GIF + 영상소스로 긴 릴스 — "직접 찍은 영상을 FFmpeg으로 합친다"
...
1. 화면 녹화 (Screen Recording)
   ...
   → Playwright로 자동 녹화 (기본)
   → 백업: Mac Command+Shift+5
   → 제품 데모, 튜토리얼에 적합
...
옵션 A: 화면 녹화 (가장 쉬움)
  Claude에게 입력:
  "현재 화면을 Playwright로 15~30초 녹화해줘.
   source.mp4로 저장해줘."
  (백업: Command+Shift+5 → source.mov)
  파일명: source.mp4
...
3. "내 손으로 만든 물건 팔기" 텍스트를 상단에 흰색 굵게 추가
...
□ 영상 소스가 세로형으로 변환됐나?
□ 텍스트 오버레이가 잘 보이나?
□ Session 2(HTML만), Session 3(+GIF)와 비교했을 때 차이가 느껴지나?
...
정답: 1번.
피드백: "맞아요! FFmpeg 명령어는 복잡하지만, Claude에게 '이 영상을 세로형으로 바꾸고 텍스트 넣어줘'처럼 설명하면 명령어를 직접 작성하고 실행까지 해줘요. FFmpeg 문법을 외울 필요가 없어요. Session 5에서는 여기에 SRT 자막과 음성을 추가해서 완전한 릴스를 완성해볼게요."
