...
Claude Code 화면 아래에 **상태 표시줄**을 커스터마이즈할 수 있다.
일일/주간 토큰 사용량, 컨텍스트 잔량을 실시간으로 확인할 수 있다.

**Claude에게 시키기:**

```
/statusline 일일 토큰 사용률과 경과시간, 주간 사용률과 경과시간, 컨텍스트 사용률을 간결하게 보여줘
예시 형식: 5h:13%(0h14m) wk:30%(1d1h) | ctx:20%
```

> 이 한 줄이면 Claude가 스크립트를 만들고 설정까지 해준다.
> 결과: `5h:13%(0h14m) wk:30%(1d1h) | ctx:20%`
>
> - `5h` = 일일 한도 기준, `13%` = 사용률, `(0h14m)` = 경과시간
> - `wk` = 주간 기준, `ctx` = 현재 대화 컨텍스트 잔량
> - 비용과 한도를 직관적으로 파악할 수 있어 실무에서 유용하다
